...
- ⚠️ **The JSON is NOT the source of truth at runtime — `ui_translations` overrides it.** `src/i18n/request.ts` merges admin-edited DB rows over the shipped JSON on every request (`getOverrides` → `mergeOverrides`, `src/lib/translations.ts`). A row exists only where an admin edited a string away from its default, and **the row wins**. There is no staleness check: nothing records which default an override was written against, so **editing `messages/*.json` can have zero effect on what users see.** That already happened — commit `e05dd48` "use one Afrikaans word for the mock exam" never reached `/af`, and all 49 override rows (48 `af`, 1 `en`, written 2026-07-17/19) still shadow every i18n commit and the claims audit since. **Before trusting any i18n change, diff the DB against the file**, then verify against the served HTML — not the file. Overrides are tag-cached (`ui-translations`, busted by `updateTag` on admin save), so a direct PostgREST write will not invalidate the cache. Plans: `docs/action-plans/AP-01` (repair), `AP-02` (make stale overrides stop applying).
