...
| **Every claim true** | ⚠️ **`/en` only.** `docs/claims-audit-2026-08-04.md` corrected 32 strings *in `messages/*.json`* — but **`/af` still serves pre-audit copy from the `ui_translations` table**, which overrides the JSON at request time. Live on production 2026-08-05: a false "5 minute" test length, two "werk aflyn" offline claims (offline does not exist), a hardcoded R179 bypassing `pricing.ts`, and the parent-consent promise the audit removed. Fix: `docs/action-plans/AP-01`+`AP-02` |
...
- ⚠️ **The JSON is NOT the source of truth at runtime — `ui_translations` overrides it.** `src/i18n/request.ts` merges admin-edited DB rows over the shipped JSON on every request (`getOverrides` → `mergeOverrides`, `src/lib/translations.ts`). A row exists only where an admin edited a string away from its default, and **the row wins**. There is no staleness check: nothing records which default an override was written against, so **editing `messages/*.json` can have zero effect on what users see.** That already happened — commit `e05dd48` "use one Afrikaans word for the mock exam" never reached `/af`, and all 49 override rows (48 `af`, 1 `en`, written 2026-07-17/19) still shadow every i18n commit and the claims audit since. **Before trusting any i18n change, diff the DB against the file**, then verify against the served HTML — not the file. Overrides are tag-cached (`ui-translations`, busted by `updateTag` on admin save), so a direct PostgREST write will not invalidate the cache. **That bit locally on 2026-08-06**: the `0631ecc` repair deleted the 8 false `af` rows straight through PostgREST, and the dev server kept serving the pre-repair Afrikaans out of `.next` (the fetch is `cache: "force-cache"` + tag) through restarts — `rm -rf .next` was the fix, and production was correct all along because the deploy built against a fresh cache. **If a translation change looks like it didn't land locally, clear `.next` before believing the DB is wrong.** Pinned by `node scripts/e2e/regression.mjs af`, which reads the served page. Plans: `docs/action-plans/AP-01` (repair), `AP-02` (make stale overrides stop applying).
