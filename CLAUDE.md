...
🔴 **Blocked: the sandbox rejects our signature.** A signed request POSTed to `sandbox.payfast.co.za/eng/process` returns *"signature: Generated signature does not match submitted signature"*. This is **not** an encoding bug — it was probed against the live sandbox across all plausible variants (spaces as `+` vs `%20`, raw `encodeURIComponent`, no encoding at all, lowercase hex, with and without passphrase, minimal 4-field request) and **every variant is rejected identically**, while the implementation matches `thephpleague/omnipay-payfast` field-for-field. The remaining explanation is the shared sandbox account: its configured passphrase is not the published `jt7NOE43FZPn`. **Fix: register a sandbox account at sandbox.payfast.co.za, set `PAYFAST_SANDBOX_MERCHANT_ID`/`_MERCHANT_KEY`/`_PASSPHRASE`, re-probe.** Until a sandbox payment completes, treat the signature path as unverified. The reusable probe scripts pattern: build a request, POST it, grep the returned HTML for the invalid-field name.
