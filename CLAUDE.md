...
| `exam_attempts` | **6 as of 2026-08-06** — 5 fixtures (the e2e buyer `129a84e0`, from `scripts/e2e/assessment.mjs`) plus **one real sitting: Louwrens** (`d9357949`, 89%, passed, 2026-08-05 17:57). The "all fixtures" line here was true for one day and stopped being true the same evening. **A real person has now sat a full paper; no paying learner from the market has.** Filter on `user_id` before reading any of it as usage — and don't touch `d9357949`'s row from a driver |
...
| **Every claim true** | ⚠️ **Superseded as a launch gate — see below. Both locales are now genuinely in sync** (`ui_translations` is empty, AP-01/AP-02 done), but the landing **deliberately** carries claims ahead of the build |
| **Human sign-off** | ✅ **274/274** as of 2026-08-05 — 46 in-app (2026-08-03), 228 by CSV batch (see below) |

⚠️ **"Every claim true" no longer governs marketing copy — John reversed it on 2026-08-07 while
the site is in beta.** Louwrens's K53-35 landing rewrite shipped **wholesale**, including claims
the build does not support: "Coach K explains every mistake" (explanations are static verified
prose), "unlimited AI Tutor" (Ask Coach caps at 25/day, 400/period), "personalised learning paths"
(weak-area tracking adapts nothing), an **Achievements** tab that does not exist, and an
unverifiable "South Africa's first". His instruction: *"You ship everything, even if false &
unsupportive, the client requested & we are still building out the site it's still very much in
BETA."* Each such claim becomes a **build item**, not a copy correction — do not re-raise them as
defects. He vetoes individually, and vetoed exactly two: **"works offline"** (no service worker
exists) and **"Parent Progress Reports"** (no schema; parent access on hold).
**This does NOT touch constraint 9 or 10** — learner content, where a wrong statement teaches the
wrong law, is still a hard accuracy gate. Marketing and learner prose are different jobs.
A claim running *behind* the build is still just a mistake to fix: his "245+ road signs" shipped
as **375+** against 380 served.
...
and the whole library are still free, so R210 buys mock exams, the AI assessment and **Ask Coach**
...
# Shared plumbing (playwright resolution, cached chromium, .env.local, the
# signed-in context, service-role PostgREST, the check harness) is scripts/e2e/lib.mjs
# — add a driver by importing it, never by copying the boilerplate again.
...
- ⚠️ **The JSON is NOT the source of truth at runtime — `ui_translations` overrides it.** `src/i18n/request.ts` merges admin-edited DB rows over the shipped JSON on every request (`getOverrides` → `mergeOverrides`, `src/lib/translations.ts`). A row exists only where an admin edited a string away from its default, and **the row wins**. Historically there was no staleness check, so **editing `messages/*.json` could have zero effect on what users see** — commit `e05dd48` "use one Afrikaans word for the mock exam" never reached `/af`, and 49 rows written 2026-07-17/19 shadowed every i18n commit and the claims audit after them. ✅ **Both halves are now closed (2026-08-07): the table holds 0 rows, and AP-02's `default_hash` column + `npm run i18n:check` fail a stale override in `CLAIM_NAMESPACES`.** The mechanism is still live, though — **one admin save re-arms it** — so re-run the check immediately before merge, not once at the start. **Three green states look identical and only one means what you want:** it exits 0 when `.env.local` is missing (it never reached the DB), and it returns early at 0 rows without hashing anything. Assert it *prints* `"0 overrides. The shipped JSON is what renders."` The hash seeds **jointly on EN+AF**, so always edit both locales in the same commit — an English-only edit flags its Afrikaans sibling as stale. **Before trusting any i18n change, diff the DB against the file**, then verify against the served HTML — not the file. Overrides are tag-cached (`ui-translations`, busted by `updateTag` on admin save), so a direct PostgREST write will not invalidate the cache. **That bit locally on 2026-08-06**: the `0631ecc` repair deleted the 8 false `af` rows straight through PostgREST, and the dev server kept serving the pre-repair Afrikaans out of `.next` (the fetch is `cache: "force-cache"` + tag) through restarts — `rm -rf .next` was the fix, and production was correct all along because the deploy built against a fresh cache. **If a translation change looks like it didn't land locally, clear `.next` before believing the DB is wrong.** Pinned by `node scripts/e2e/regression.mjs af`, which reads the served page. Both plans are done: `docs/action-plans/AP-01` (repair), `AP-02` (make stale overrides stop applying).
...
3. **Under-18 users are expected (overview §11).** Target market includes Grade 11/12 learners. The free readiness test is anonymous and **device-local** — the result is persisted only in the browser via `localStorage` (`src/lib/storage.ts` `saveReadinessResult`/`loadReadinessResult`), so an unregistered learner keeps their score across sessions with nothing sent to a server. This is distinct from **server-side** persistence of a minor's personal progress (the `readiness_results` table), which still requires sign-in + parent/guardian consent. Keep the payment screen parent-facing; collect minimal PII.
    ...
    ✅ **An anonymous result deliberately does NOT carry over to a new account (John, 2026-08-06) — do not "fix" this.** Signing up starts a fresh account and pre-account test results do not count. `readiness_results` is written only by `readiness/result/page.tsx`, and only while the learner is on that page *and* already signed in — so taking the test anonymously and then registering leaves nothing behind. That is intended, not the gap it looks like. It has been raised once as a bug already; the answer is recorded here so it stops being re-raised.
...
5. **Pricing model is once-off, not subscription — and there is exactly ONE price (John, 2026-08-07).** **R210 once-off for 90 days** full access; schools **R180/learner/90 days**. Constants live in `src/lib/pricing.ts` (`ENTITLEMENT_PRICE_ZAR`, `SCHOOL_PRICE_ZAR`) — never hardcode either, least of all inside a translated string, where a literal price is a claim no constant change can correct. KPIs are framed around free-test→paid-unlock conversion and parent share rate, **not** "subscription conversion" (the PRD's original wording is superseded).

    ⚠️ **Two figures that stood here for months are now WRONG — don't restore them from an older summary.** The **R149–R199 band** is superseded: R210 sits above it deliberately, so a price outside that band is not a defect to report. And the **optional R20/month AI Coach tier does not exist and is not coming** — Louwrens proposed R80/month + R35/month in K53-35 and John killed both outright ("there's one price"). Nothing in the schema supports a recurring charge (`entitlements.product` is `check (product in ('full_access'))`, `PAYMENT_FIELD_ORDER` carries no `subscription_type`/`frequency`/`cycles`, and there is no cancellation path), and five shipped strings plus the landing headline **"No subscription trap"** rest on its absence. Adding a monthly tier is a migration + a PayFast recurring integration, not a config change.

    ⚠️ **The ITN handler rejects any amount ≠ `ENTITLEMENT_PRICE_ZAR`** (`src/app/api/pay/payfast/route.ts`, ±R0.01). So a discount coupon does not merely go ungranted — it is refused with a 400 while the learner has paid. There is no coupon/voucher table anywhere; Louwrens's R30 school voucher is unbuilt. Changing the price changes what the gateway will accept, in the same commit, by design.
